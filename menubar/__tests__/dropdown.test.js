...
const EXPECTED = [
  'claude-sonnet-5','claude-opus-5','claude-haiku-4-5','gemini/gemini-3.6-flash','gemini/gemini-3.1-flash-lite','gpt-5.6-terra','gpt-5.6-luna',
  'claude-fable-5','claude-opus-4-8','claude-opus-4-7','claude-sonnet-4-6',
  'gemini/gemini-3.5-flash','gemini/gemini-3.1-pro-preview','gemini/gemini-2.5-pro','gemini/gemini-2.5-flash',
  'gpt-5.6-sol','gpt-5.5','o3',
  'gpt-5.3-codex','gpt-5.5-pro',
];
const values = [...selBody.matchAll(/<option value="([^"]+)"/g)].map(m => m[1]);
assert.strictEqual(values.length, 20, `expected 20 options, got ${values.length}`);
...
console.log(`dropdown OK: ${values.length} options, ${groups} groups,`, DEFAULT_MODEL, 'selected');

...
- **(10) MATRIX — RELEASE BLOCKER:** run the smoke on **native arm64 and x86_64**; assert the venv's installed set equals the **checked-in expected manifest** for that `(python-minor, arch)`. Only `cp310/cp312/cp313-arm64` are covered today; **cp311, cp314, and every x86_64 cell fail closed** (provisioning refuses, no fallback). Before production release, either generate the missing cells (`menubar/scripts/pydeps.js`, run on native/equivalent hosts) or **narrow the supported interpreter matrix** and state it in `SETUP.md`. See `resources/pydeps/README.md`.

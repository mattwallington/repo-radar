'use strict';
// Codex R6 (R6-4 / Ruling 56): charge-arithmetic parity between Python's quota.py
// `_compute_snapshot` (the source of the shared `accounting_vectors.json` fixture) and Node's
// quota.js `_computeSnapshot`. Each vector is the already-gathered accounting INPUT (root/ledger
// listability, per-activity on-disk sizes with `null` for unmeasurable, rejected root ids, and
// the ledger entries) plus the constants it was computed under and the `{ charge, uncertain,
// corrupt }` Python produced. Node must agree on every field -- the function is PURE, so this is
...
// output shape (camelCase). Field-for-field, no interpretation.
function toNodeInputs(inp) {
  return {
    rootListable: Boolean(inp.root_listable),
    ledgerListable: Boolean(inp.ledger_listable),
    activities: (inp.activities || []).map((a) => ({ aid: a.aid, onDisk: a.on_disk === null ? null : a.on_disk })),
...
if (!fs.existsSync(VECTORS)) {
  ...
  for (const v of vectors) {
    test(`accounting parity: ${v.name}`, () => {
      const got = quota._computeSnapshot(toNodeInputs(v.inputs), v.constants);

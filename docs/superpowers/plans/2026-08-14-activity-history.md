...
- **Configured-secret source + propagation (Round-3 #4) — the API must be WIRED, not just present.** The effective secret set = the GitHub token + Anthropic/Gemini/OpenAI API keys held in the app config. **Python** sources them from `repo_radar.config.load_config()` and passes them to `ActivityWriter(configured_secrets=...)` in `cli.py` and `sync_mode` (Tasks 2.5/2.6). **Electron** sources them from its `config.json` loader and passes them to the Node write-side `ActivityWriter` (Task 2.3) **and** to the Node read / System-diagnostics / IPC / export `Redactor` (Tasks 3.4/3.6/4.1/4.3). Each entry path carries a test using a **non-pattern-matching** configured secret value (e.g. a plain high-entropy string), proving the wiring — not merely the regex algorithm.
...
2. **The POSIX-sh dispatcher stays thin — it never reimplements records/lease/quota.**
   - **Scheduled path:** the shell mints `activity_id`+`owner_token`, **acquires+holds** the `owner.lock` fd (same pattern as the root `.exec.lock` on fd 9), then invokes `python -m repo_radar.activity.bootstrap` **before** the runtime-verify guard; bootstrap (no existing `start` → first producer) does `admit` + writes the single `start` + initial `ownership`. That interpreter still *executes* even when the fingerprint guard will later *reject* it (the motivating failure).
   - **Manual path (id inherited from Electron):** the shell does **not** mint or bootstrap — it carries Electron's fd (→ child fd 4) and `exec`s python, whose executing owner **adopts** the lease and writes the handoff `ownership` (Electron's ack).
   - **A bootstrap that fails to adopt exits non-zero and writes nothing (no false ack) but does NOT abort the sync** — observability is best-effort; the lease is held by the shell/exec'd fd and the reconciler finalizes.
   - On a dev/verify guard failure the shell calls `python -m repo_radar.activity.finalize` (`blocked` + settle + release). **Root-lock contention finalizes `skipped` with exactly one authority** — the dispatcher on the scheduled path, Electron on the manual path (Round-3 #2).
   - **If the venv Python cannot execute at all** (rare hard corruption), the shell writes ONE bounded, redacted line to the **System diagnostic stream** (`sync.error.log`) — never an un-quota'd Activity segment (Round-1 #1).
...
## Plan-review Round 2 dispositions (all accepted, plan rev 3 @ `fe5289c`)

| # | Finding | Fix |
|---|---------|-----|
| 1 | quota not the real crash-recoverable machine | `reconcile.py` `synthesize_terminal`; `admit` reconciles before charging; parsed top-level types; lock-safe `prune`; concurrent-admission test |
| 2 | cross-writer reserve slots | slot check under the writer mutex + exclusive cancellation authority (only the minter) |
| 3 | never-raises init/finalization | inactive-on-failure writer; independent settle+release |
| 4 | negative handoff | bootstrap non-zero + no false ack; Electron never unlocks under a live child |
| 5 | Task 2.7 forward dep | crash case uses the Phase-1 Python reconciler |
| 6 | fs hardening + lock path | fd-based fchmod + repair; `activity/quota.lock` |
| 7 | record conformance | enum validation; nested redaction; `boot_id`/`proc_birth` |
| 8 | reader/UI/stale contracts | tri-state probe; System limits; `{available,incomplete}`; stale docs removed |

## Plan-review Round 3 dispositions (all accepted, this revision)

| # | Finding | Fix |
|---|---------|-----|
| 1 | façade can still unlock/leak/throw | adopted-cleanup = drop-only; all payload build inside `_emit`; independent settle+release proven FREE; idempotent `start` + `terminal` ensures one start |
| 2 | double terminal on manual root contention | exactly one authority (Electron manual / dispatcher scheduled); `onContention` no double release |
| 3 | ack timeout kills a healthy sync | post-spawn Electron never kills + never terminals; drop descriptor + reconcile; sync always completes |
| 4 | configured-secret redaction not wired | source (`load_config` / `config.json`) + propagation pinned into every entry path + non-pattern secret tests |
| 5 | Task 2.2 commit omits `reconcile.js` | added to the boundary |
| 6 | lifecycle parsing / quota cleanup | shared `records.parse_valid` (v1 + activity_id + required fields); ledger full-invariant validation; prune removes `owner.lock` + whole dir |
| 7 | not descriptor-relative | `dir_fd`+`O_NOFOLLOW` component walk; repair owned tree only; intermediate-symlink test |
| 8 | numeric canonicalization | integral floats → int; non-finite rejected both languages; golden numeric case |
...
## Plan-review Round 5 dispositions (all accepted, plan rev 6 @ `d79d9eb`)

| # | Finding | Fix |
|---|---------|-----|
| 1 | ledger atomic-not-durable + pathname ops | descriptor-relative durable `_write_entry` (temp+full-write+fsync+atomic-rename+dir-fsync); `quota.lock`/settle/reconcile-unlink `dir_fd`-relative; admit/grant refuse on swapped component |
| 2 | `_started` = attempted | 3-state `_emit`; `_start_written` set only on real write; no empty-resync terminal-only |
| 3 | validator accepts malformed authority records | complete v1 shape validation both langs + shared vectors |
| 4 | early exit ≠ rejection | `HANDOFF_REJECTED_EXIT=66` is the only `failed` signal; other exits → reconcile |
| 5 | Node deletion unsafe | all destructive deletion delegated to Python; Python opens reject non-regular |
| 6 | Task 3.3 tests vs canonical parser | valid seeds + all-segment enumeration |
| 7 | truncation parity not executable | committed `golden-truncation.json` asserted in-process both langs |

## Plan-review Round 6 dispositions (all accepted, this revision)

| # | Finding | Fix |
|---|---------|-----|
| 1 | `_emit` seq/partial/zero-byte | seq consumed on a COMPLETE line (independent of fsync) so retry has strictly-increasing seq; partial line `ftruncate`d away; zero-byte write → error; +start recovery tests |
| 2 | strict validation breaks fixtures | all fixtures use valid ISO ts + 8-hex `by`; ownership requires `pid`/`boot_id`/`proc_birth`; `parse_valid` rejects non-finite + validates an actual timestamp; vectors extended |
| 3 | prune ownership / task ordering | Phase-1 `prune.py` entrypoint; Node admit release→invoke-Python→reacquire; retention impl (3.5) vs cadence wiring (5.2) split; Phase-3 gate runs Python too |
| 4 | unsafe-file open can block | `open_owned_regular` = `O_NONBLOCK`+`O_NOFOLLOW`+`S_ISREG`; lease acquire/probe/adopt routed through it; FIFO no-block tests |
| 5 | ledger edge cases | exact non-boolean int counters; zero-byte error; temp cleanup on rename failure; UUID-validate ledger names |
| 6 | Phase-3 placeholder tests | concrete UNCERTAIN (probe seam) + full retention-matrix tests |
...
[
  ...
  {"accept": true,  "why": "valid ownership w/ evidence", "raw": {"schema_version":1,"activity_id":"00000000-0000-4000-8000-000000000000","type":"ownership","seq":1,"ts":"2026-08-14T00:00:00-07:00","owner_token":"deadbeef","role":"initial","producer":"python","pid":1234,"boot_id":"a1b2c3d4e5f60718","proc_birth":"2026-08-14T00:00:00-07:00"}},
  {"accept": false, "why": "ownership missing pid/boot_id/proc_birth", "raw": {"schema_version":1,"activity_id":"00000000-0000-4000-8000-000000000000","type":"ownership","seq":1,"ts":"2026-08-14T00:00:00-07:00","owner_token":"deadbeef","role":"initial","producer":"python"}},
  {"accept": false, "why": "non-ISO calendar ts (regex-shaped but invalid)", "raw": {"schema_version":1,"activity_id":"00000000-0000-4000-8000-000000000000","type":"event","seq":0,"ts":"2026-13-40T99:00:00-07:00","level":"info","event":"x","fields":{}}}
]
```
...
Run: `python -m pytest repo_radar/tests/test_activity_lease.py -v`
Expected: PASS. §5 truth table (independent-busy AND inherited-reassert-succeeds); **add FIFO/device, permission, and intermediate-symlink refusal tests for `owner.lock`** (Round-6 #4), each with a timeout to prove no blocking.

- [ ] **Step 5: Commit**

```bash
git add repo_radar/activity/lease.py repo_radar/tests/test_activity_lease.py
git commit -m "feat(activity): flock lease — §5 validation + descriptor-relative regular-file, FIFO-safe"
```

### Task 1.6: Crash-recoverable admission-lock quota ledger

**Files:**
- Create: `repo_radar/activity/quota.py`, `repo_radar/activity/reconcile.py`, `repo_radar/activity/prune.py` (the **admission-prune entrypoint**, in Phase 1 so Node's Phase-2 admission can invoke it — Round-6 #3)
- Test: `repo_radar/tests/test_activity_quota.py`, `repo_radar/tests/test_activity_reconcile.py`

`prune.py` (`python -m repo_radar.activity.prune`): under `quota.lock`, `reconcile` then repeatedly `_prune_locked` until `charge ≤ CEILING` (or nothing prunable); prints the freed byte count. This is the ONLY destructive deletion path Node uses at admission (descriptor-relative, race-free — Round-6 #3/#5).
...
**Interfaces:**
- Produces (CommonJS mirror of Python write side): `acquire(lockPath)`, `adopt(inheritedFd, ownerToken, lockPath)`, `probe(lockPath)` (tri-state), `probeBusy(lockPath)`, `admit`, `grant`, `settle`, `reconcile`(synthesize), `ActivityWriter` (Electron manual path: `start`, `event`, `control`, `terminal`, `handOffEnv()`, `dropLocalReference()`, `activityId`, `_handedOff`). **Node NEVER deletes (Round-6 #3/#5):** when `admit` finds the charge over the ceiling it **releases `quota.lock`, spawns `python -m repo_radar.activity.prune`, then re-acquires the lock and re-evaluates** — it must **not** hold the Node lock while the Python helper obtains it. Node holds the lease via a retained fd + `/usr/bin/lockf -t 0 <fd>` (spec §5); `probe` runs `/usr/bin/lockf -t 0` on a fresh fd and maps exit 0→FREE, 75→BUSY, other→UNCERTAIN. **JSON ledger (Decision 3), lifecycle derived from segments, per-activity cap in `grant`, partitioned one-shot reserve — byte-for-byte the Python contract.**
...
```bash
git add menubar/activity/lease.js menubar/activity/quota.js menubar/activity/writer.js menubar/activity/reconcile.js menubar/activity/__tests__/ repo_radar/tests/test_activity_behavior_vectors.py
git commit -m "feat(activity): Node lease/quota/writer/reconcile(synth) + bidirectional lock interop + shared behavioral vectors"
```
...
- The generated `_script`:
  ...
  - **On root exec-lock contention** (the existing fd-9 `.exec.lock` returns exit 75): **scheduled path only** → `python -m repo_radar.activity.finalize --outcome skipped`, then exit 75. **Manual path (id inherited): do NOT finalize** — exit 75 and let **Electron** finalize `skipped` via its `runSync` reject handler, so manual contention has **exactly one** terminal authority (Round-3 #2 — no double terminal/settlement).
...
test('invalid-outcome OR foreign-activity terminal is NOT a valid record (Round-4 #5)', () => {
  ...
  assert.strictEqual(records.filter(r => r.type === 'terminal').length, 0);   // bogus outcome + foreign id
});
```
...
**Files:** Add `retain(home) -> [pruned_ids]` to `repo_radar/activity/quota.py` (builds on `_classify` + `_prune_locked`'s `unlink_owned_tree`, all under `quota.lock`) + `repo_radar/activity/retain.py` (`python -m repo_radar.activity.retain`). **The Electron cadence wiring is Task 5.2** — kept separate (Round-6 #3). Test `repo_radar/tests/test_activity_retain.py`. **The Phase-3 gate runs the Python suite (`python -m pytest`) as well as `node --test`,** since this task adds Python code.
...
- [ ] **Step 1: Failing tests (Python)** — one per matrix row, with concrete setup (Round-6 #6):

```python
# repo_radar/tests/test_activity_retain.py
import json, os, time
from repo_radar.activity import quota, paths, lease, ids
...
- [ ] TDD:
  ...
  - **`get`/`reveal` path-safety tests:** an unsafe/invalid `activity_id` (`../…`, non-UUIDv4) is **rejected** by the handler and never used to build a path; `reveal` only ever reveals a validated path under `activity/`.
...
- [ ] TDD:
  ...
  - **Three-state renderer assertions (Round-3 #9)** against the pure render helper: `{available:true, items:[]}` → the **empty-history** state; `{available:true, incomplete:true, items:[…]}` → items **plus** an incomplete banner; `{available:false}` → the **unavailable** message (distinct text from empty).
  - implement; commit `feat(activity): tray Activity entry + redacted export + three-state unavailable/incomplete UI`.

...
- **Capacity reclamation (coalescing conservative chunks to cut paid calls) is a non-goal** for this branch — correctness only. Enforced **mechanically**: Branch 1's `repo_needs_chunking`/synthesis partition is the coarsest allowed; authoritative counting may only split/tighten, never merge or reverse chunk→single.

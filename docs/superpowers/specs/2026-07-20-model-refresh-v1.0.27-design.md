...
### 2.5 MODEL_MIGRATIONS — literal, deterministic (unavailable-only; provider+tier preserved)
...
**OpenAI:** `o1-preview`, `o1-mini` → `o3`; `gpt-5.2-codex` (and any other pre-5.3 `*-codex` ID the §8 gate confirms shut down by release — currently `gpt-5-codex`, `gpt-5.1-codex`, `gpt-5.1-codex-max`, `gpt-5.1-codex-mini`) → `gpt-5.3-codex`. Each such codex key is **removed from KNOWN_LIMITS** in the same change.
...
**Shutdown gate** (new): a check (in `release.sh` preflight or a dedicated test run by it) that queries the OpenAI/Anthropic/Google deprecation data and **fails the release** if (a) any KNOWN_LIMITS ID is no longer callable, or (b) any `MODEL_MIGRATIONS` key is unexpectedly still active. Drift → human amends the spec/map.

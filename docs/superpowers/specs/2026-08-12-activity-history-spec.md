...
**Status:** Spec — decision review (paired, rev 4). Scope **B**, full vertical MVP through the Activity window.
**Date:** 2026-08-12
**Builds on (approved):** `2026-08-12-log-viewer-shape.md` @ `4972b4e`. Resolves the deferred decisions per paired Rounds 3–7.
...
- **One logical lease — exactly one *open-file description* at a time.** Its fd may be **transiently duplicated** across the parent/child boundary during a handoff; that is still one lease, not two. `owner_token` is minted when the descriptor is first locked and names that one lease for the `terminal`'s `by`.
  - **Electron** acquires the lock (minting `owner_token`), writes `start` + the **initial `ownership`**, holds it across the synchronous guards, then either finalizes+releases (guard block/contention) or **passes the locked descriptor + `owner_token` to the spawned dispatcher** (fd number + token in the child env). It closes its own copy **only after** the dispatcher **validates** the inherited descriptor — that it refers to *this* activity's `owner.lock` (fd `fstat` identity == a fresh `stat` of the path, and the lock is held) — and writes its **handoff `ownership`** (same `owner_token`). **Failed validation/handoff** leaves **Electron authoritative** to finalize `failed` and release; the child does not proceed as owner.
  - **The dispatcher** inherits + validates that descriptor (or, when scheduled, acquires its own + mints its own token), writes the handoff `ownership`, and `exec`s Python (`dispatchers.js:109`); the descriptor + token survive `exec`, so the same held lease passes into Python — no dispatcher→Python re-acquire.
...
- **Global ceiling 64 MiB — enforced by an owner-only admission lock, not hoped for.** A single **admission lock** `activity/quota.lock` (BSD `flock`, held only for brief accounting critical sections) serializes all global-budget accounting against a small persistent ledger of `used_bytes` + `reserved_bytes`. This is what makes 64 MiB a real invariant across the independently-spawned Electron/dispatcher/CLI producers rather than a value each races to observe and then blow past together.
  - **Admission (under the lock, before `start`):** the activity reserves a **60 KiB** finalization reservation (added to `reserved_bytes`). If `used_bytes + reserved_bytes + 60 KiB` would exceed 64 MiB, the reconciler prunes prunable items first (order below); if still no room, **admission fails best-effort** — ONE non-recursive warning, the activity lease is released, Activity recording is skipped, **and the sync proceeds unchanged** (observability never blocks execution).
  - **Ordinary writes** (`start`, `event`) take a **byte-grant under the admission lock** before appending — a writer may take a bounded **batched** grant and spend it locally to avoid per-line locking, but every grant is accounted, so `used_bytes + reserved_bytes` can never cross the ceiling.
  - **Reserved writes** (`control`, `terminal`, `integrity`) draw from the activity's **own 60 KiB reservation** — already counted at admission — so they take **no** global-lock grant and can **never** be refused. Finalization cannot deadlock, and because the bytes were pre-reserved there is **no unbounded overshoot**.
  - **Release (under the lock):** once the `terminal` is durable, the holder/reconciler reconciles the ledger (drop this activity's residual reservation, fold its real bytes into `used_bytes`).
...
1. **Foundation:** record contract + storage layout + identity/validation + schema versioning + **advisory-lock lease (with `owner_token` mint/handoff) + global admission lock + quota ledger + finalization state machine + lock-probe reconciliation** + at-rest bounds (partitioned 60 KiB reservation) + best-effort write behavior (Python + a minimal writer). Unit-tested + a CLI-driven attempt.
...
- Admission/ledger: concurrent admissions under `quota.lock` never let `used_bytes + reserved_bytes` exceed 64 MiB (N independent producers admitting at once stay bounded); a failed admission is best-effort (warns once, releases the lease, sync proceeds); the reservation is released into `used_bytes` after the terminal is durable.

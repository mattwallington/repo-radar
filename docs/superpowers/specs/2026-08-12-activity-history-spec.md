...
**Status:** Spec — decision review (paired, rev 5). Scope **B**, full vertical MVP through the Activity window.
**Date:** 2026-08-12
**Builds on (approved):** `2026-08-12-log-viewer-shape.md` @ `4972b4e`. Resolves the deferred decisions per paired Rounds 3–8.
...
- **One logical lease — exactly one *open-file description* at a time.** Its fd may be **transiently duplicated** across the parent/child boundary during a handoff; that is still one lease, not two. `owner_token` is minted when the descriptor is first locked and names that one lease for the `terminal`'s `by`.
  ...
  - **Inherited-descriptor validation (exact, required before adopting):** the adopter (1) checks the inherited **fd number** and `owner_token` are syntactically valid; (2) verifies `fstat(fd)` matches a fresh **non-symlink** `stat` of *this* activity's `owner.lock` (same dev+inode); (3) **re-asserts the exclusive lock on the inherited fd itself** — `fcntl.flock(fd, LOCK_EX|LOCK_NB)` (Python) / `/usr/bin/lockf -t 0 <fd>` (shell/Node) — which **succeeds** only because the inherited descriptor shares the original open-file description that already holds the lock, whereas a fresh *independent* probe returns exit 75. Identity alone is insufficient: a fresh-path probe showing the file locked does **not** prove *this* descriptor carries the lock (another process could hold it). Only on all three does the adopter append the handoff `ownership` and acknowledge Electron; **any failure rejects the handoff** (Electron stays authoritative, finalizes `failed`).
...
- **Global ceiling 64 MiB — enforced by an owner-only admission lock over a crash-recoverable, activity-addressable ledger.** A single **admission lock** `activity/quota.lock` (BSD `flock`, brief critical sections) serializes all budget accounting. **Committed usage is authoritative from the filesystem, not a running total a crash could desync:** under the lock, `committed = Σ actual byte-sizes of every `activity/*/*.jsonl` segment` (a bounded scan — item count is ceiling-bounded). The **ledger** — dir `activity/quota/` (`0700`), per-activity file `activity/quota/<activity-id>.json` (`0600`, written atomically via temp+rename) — holds, for each **live, un-settled** activity only, its **outstanding** `{reserved, granted}`: the headroom not yet materialized on disk. **Accounted unit:** logical segment bytes **including newlines**; the bounded bookkeeping files (`quota.lock`, ledger entries) are **outside** the 64 MiB payload ceiling.
  ...
  - **Reconcile under the lock — at startup, before admission, and on any torn/stale entry** — from the same bounded scan: a ledger entry whose activity has **no `start`** (abandoned reservation) is **released**; an entry whose activity has a **durable `terminal`** is **settled** (entry removed); a **torn/corrupt** entry or otherwise ambiguous state is charged **conservatively** (its full `reserved + granted`) and **refuses new history writes best-effort rather than undercounting**, then is rebuilt atomically.
...
- Admission/ledger (crash recovery): committed usage is scan-authoritative, so grant-before-append means a mid-write crash **overcounts, never undercounts**; reconcile releases an abandoned reservation (entry with no `start`), settles an entry whose activity has a durable `terminal`, and on a torn/corrupt entry charges conservatively + refuses new writes best-effort; prune-before-decrement (delete files first) can only overcount transiently; the ledger/lock bookkeeping bytes are outside the 64 MiB payload ceiling.
...
- Inherited-descriptor validation: the adopter **re-asserts `flock` on the *inherited* fd** and it succeeds (shared open-file description), while a fresh *independent* probe of the same `owner.lock` returns exit 75; identity match (`fstat`==`stat`) alone does **not** cause adoption; any validation step failing rejects the handoff.
